...
public class ShowFPS : MonoBehaviour
{
    ...
    [Tooltip(
        "Number of frames to average together in frame rate statistics (1 corresponds to no averaging)"
    )]
    [Range(1, 999)]
        ...
        if (lastRealtime > 0.0f && time > 0.0f)
        {
            ...
            if (deltaRealtime >= 0.0f && deltaTime >= 0.0f)
                ...
                GUI.Label(
                    ...
                    "Render FPS: "
                        + (1.0f / deltaRealtime / 2.0f).ToString("0.00")
                        + " ("
                        + (deltaRealtime * 1000 * 2).ToString("0.00ms")
                        + ")\n"
                        + "Game FPS: "
                        + (1.0f / deltaTime / 2.0f).ToString("0.00")
                        + " ("
                        + (deltaTime * 1000 * 2).ToString("0.00ms")
                        + ")",
                    btnStyle
                );
        }
